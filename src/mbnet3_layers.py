...
# ==============================================================================
"""Implementation of paper Searching for MobileNetV3,
...
class Bneck(tf.keras.layers.Layer):
    ...
    def call(self, input):
        ...
        # can do residual?
        res_ok = self.stride == 1 and self.in_channels == self.out_channels
        if self.let_res and res_ok:
            print('[warning]: count residual connection!')
            return input + x
        else:
            return x
...
class LastStageBinary(tf.keras.layers.Layer):
    def __init__(
            self,
